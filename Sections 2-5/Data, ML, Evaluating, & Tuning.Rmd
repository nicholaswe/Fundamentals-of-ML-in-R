...
```
Response variable is missing, we need to get it back.

```{r Restore response variable}
train_data_caret$churn <- train_data$churn
test_data_caret$churn <- test_data$churn
```
...
churn_recipe <- recipe(churn ~ ., data=train_data) %>%
  ...
  step_normalize(all_numeric_predictors()) # combines scale+center
```
